...
如果你不需要打开网页，可以把 `data/daily-brief.json` 渲染成一段适合微信阅读的文本：

```bash
python scripts/render_daily_brief.py --limit 10
```

如果本机已经配置好 Hermes Weixin 网关，可以直接发送到微信：

```bash
python scripts/send_weixin_brief.py --limit 10
```

建议先 dry-run 预览，不触发真实发送：

```bash
python scripts/send_weixin_brief.py --limit 10 --dry-run
```

这条路径只使用已经生成的伯乐精选 `daily-brief.json`，不会读取或提交私有 OPML、cookies、token 或邮箱正文。
